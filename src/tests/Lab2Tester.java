...
//	            	mainStudent.update();
					...
					System.out.println("Updated " + mainStudent.update() + " records.");
...
//	            		+ " verify the new record exists in pgAdmin by running the \"SELECT * FROM Students;\" command ");
				...
				try{
					PreparedStatement check = c.prepareStatement("SELECT * FROM Students WHERE userid = 100222222; ");
					if (check.execute()) {
